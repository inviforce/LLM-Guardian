...
NOTE on LOAD_TRAJECTORY:
  Set the environment variable LOAD_TRAJECTORY=false to skip loading the
  Qwen2.5-1.5B trajectory model. This is intended for memory-constrained
  free-tier hosting (e.g. Render free web services, ~512MB RAM) where
  loading all three pipelines simultaneously would exceed available memory.
...
# Whether to load the Qwen2.5-1.5B trajectory model at startup.
# Set LOAD_TRAJECTORY=false in the deployment environment (e.g. Render free
# tier) to keep memory usage within a ~512MB-1GB budget.
LOAD_TRAJECTORY = os.environ.get("LOAD_TRAJECTORY", "true").lower() == "true"
...
class LLMGuardian:
    def __init__(self, model_paths: Dict[str, Path] = MODEL_PATHS, device: str = DEVICE):
        ...
        print(f"[*] LOAD_TRAJECTORY = {LOAD_TRAJECTORY}")
        ...
        if LOAD_TRAJECTORY:
            ...
            print("[*] Skipping Qwen trajectory model load (LOAD_TRAJECTORY=false)")
            self.tokenizer_qwen = None
            self.qwen_lora = None
            # Explicitly mark as "not loaded" rather than "failed" for clarity in /health,
            # but keep the same load_status key so health reporting stays consistent.
            self.load_status["qwen_lora"] = False
    ...
    def health(self) -> Dict[str, Any]:
        return {
            ...
            # partial (free-tier) deployment instead of always "degraded".
            "all_ready": all(
                v for k, v in self.load_status.items()
                if LOAD_TRAJECTORY or k != "qwen_lora"
            ) if self.load_status else False,
        }
    ...
    def _run_qwen_trajectory(self, messages: List[Dict[str, str]]) -> Dict[str, Any]:
        if self.qwen_lora is None or self.tokenizer_qwen is None:
            return {
                ...
                "raw_generated_text": (
                    "Trajectory pipeline is disabled on this deployment "
                    "(LOAD_TRAJECTORY=false) due to free-tier memory constraints. "
                    "Run locally from the repository with LOAD_TRAJECTORY=true "
                    "for the full three-pipeline system."
                ),
                "latency_ms": 0.0,
            }
